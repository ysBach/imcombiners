...
The same fused path is available from Standard Combiner as `cmb.combine(method, rejectors=..., diagnostics=None)` after `cmb = Combiner(arr)`. The compact ndcombine() wrapper is used here because this benchmark is the IRAF-style compatibility/CLI-facing path. See [How To Get Maximum Performance](max-performance.qmd#scope-of-the-fastest-paths) for what the fast path skips and why it is faster than diagnostic routes.
