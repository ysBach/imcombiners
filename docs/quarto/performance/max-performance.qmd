...
constants. See [2-D Image Benchmarks](image-benchmarks.qmd) and
[1-D Array Benchmarks](array-1d-benchmarks.qmd) for recorded benchmark tables.
...
Several Rust statistics crates (`ndarray-stats`, `argminmax`, `scirs2-stats`) were checked making kernels in this repo. A small benchmark on `(N, 384, 384)` `float32`stacks measured the median combine core and a simple min/max scan:
...
For `min_1d` and `max_1d`, the 1-D min/max threshold is compared with
`values.size`.
