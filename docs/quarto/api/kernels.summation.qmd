# kernels.summation { #imcombiners.kernels.summation }

```python
kernels.summation(arr, *, validate=True)
```
...
<code>[**sum**]{.parameter-name} [:]{.parameter-annotation-sep} [(ndarray, shape(*spatial))]{.parameter-annotation}</code>
