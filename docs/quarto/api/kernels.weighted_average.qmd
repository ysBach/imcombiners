# kernels.weighted_average { #imcombiners.kernels.weighted_average }

```python
kernels.weighted_average(arr, weights, *, validate=True)
```
...
<code>[**weighted_average**]{.parameter-name} [:]{.parameter-annotation-sep} [(ndarray, shape(*spatial))]{.parameter-annotation}</code>
