...
- Combine methods: mean, median, lower median (`lmed`), percentiles, sum, min, max, variance, and weighted average (`wvg`).
...
- [Scope Of The Fastest Paths](performance/max-performance.qmd#scope-of-the-fastest-paths) explains when Standard Combiner, `ndcombine()`, Chained Combiner, and direct kernel calls differ in requested work.
...
Recorded local benchmark tables are split into [2-D Image Benchmarks](performance/image-benchmarks.qmd) and [1-D Array Benchmarks](performance/array-1d-benchmarks.qmd).
