...
:   Image stack. Accepted dtypes are `uint8`, `uint16`, `int16`, `int32`, `float32`, and `float64`. Pure stack reductions use `reducers`: integer `mean`, `median`, `sum`, and `variance` produce floating results, while integer `min`, `max`, and `lmedian` preserve integer dtype. Masking, rejection, zero/scale, and fused-kernel workflows use imcombiners' floating workspace (`uint8`, `uint16`, and `int16` to `float32`; `int32` to `float64`). Other dtypes, including `int64` and `float128`, are not silently cast. Inputs with more than 3 dimensions are supported and outputs are reshaped back to the original trailing spatial dimensions.
