...
* Variance is available as `Combiner(...).variance(ddof=...)` or the aliases `combine="variance"` / `"var"`. It is computed from final valid values: `np.nanvar(arr_eff, axis=0, ddof=ddof)`. Use `np.sqrt(var)` if an error or standard-deviation map is needed.
