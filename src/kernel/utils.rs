...
const DEFAULT_MINMAX_1D_PARALLEL_THRESHOLD: usize = 1_000_000;
static PARALLEL_THRESHOLD: AtomicUsize = AtomicUsize::new(0);
static MINMAX_1D_PARALLEL_THRESHOLD: AtomicUsize = AtomicUsize::new(0);
...
pub fn minmax_1d_parallel_threshold() -> usize {
    let current = MINMAX_1D_PARALLEL_THRESHOLD.load(Ordering::Relaxed);
    if current != 0 {
        return current;
    }
    let threshold = env::var("IMCOMBINERS_1D_MINMAX_PARALLEL_THRESHOLD")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .filter(|&value| value > 0)
        .unwrap_or(DEFAULT_MINMAX_1D_PARALLEL_THRESHOLD);
    let _ = MINMAX_1D_PARALLEL_THRESHOLD.compare_exchange(
        0,
        threshold,
        Ordering::Relaxed,
        Ordering::Relaxed,
...
pub fn set_minmax_1d_parallel_threshold(threshold: usize) {
    MINMAX_1D_PARALLEL_THRESHOLD.store(threshold, Ordering::Relaxed);
}
