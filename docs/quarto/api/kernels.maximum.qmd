# kernels.maximum { #imcombiners.kernels.maximum }

```python
kernels.maximum(arr, *, validate=True)
```
...
<code>[**maximum**]{.parameter-name} [:]{.parameter-annotation-sep} [(ndarray, shape(*spatial))]{.parameter-annotation}</code>
