# kernels.minimum { #imcombiners.kernels.minimum }

```python
kernels.minimum(arr, *, validate=True)
```
...
<code>[**minimum**]{.parameter-name} [:]{.parameter-annotation-sep} [(ndarray, shape(*spatial))]{.parameter-annotation}</code>
